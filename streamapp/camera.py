...
class MaskDetect(object):
	...
	def get_frame(self):
		...
		for (box, pred) in zip(locs, preds):
			...
			(mask, tengah, withoutMask) = pred

			# determine the class label and color we'll use to draw
			# the bounding box and text
			print(pred)
			if mask > withoutMask and mask > tengah:
				label = "Pakai Masker" 
				color = (0, 255, 0)
			elif tengah > withoutMask and tengah > mask:
				label = "Tidak Pakai Masker" 
				color = (0, 0, 255)
			else:
				label = "Pakai Masker Namun Belum Benar Pemakaiannya"
				color = (255, 0, 0)
...
class LiveWebCam(object):
	...
	def get_frame(self):
		...
		resize = cv2.resize(imgNp, (48, 48), interpolation = cv2.INTER_LINEAR) 
